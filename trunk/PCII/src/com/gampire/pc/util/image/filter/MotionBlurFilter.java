...
public class MotionBlurFilter extends AbstractBufferedImageOp {
	...
	public BufferedImage filter(BufferedImage src, BufferedImage dst) {
		...
		float maxDistance = distance + Math.abs(rotation * imageRadius) + zoom
				* imageRadius;
		int repetitions = Math.abs((int) maxDistance);
		...
		if (premultiplyAlpha)
			unpremultiply(outPixels, 0, inPixels.length);

		setRGB(destination, 0, 0, width, height, outPixels);
		return destination;
	}
	 ...
	 */
	private static void premultiply(int[] p, int offset, int length) {
		int lenghtPlusOffset = length + offset;
		for (int i = offset; i < lenghtPlusOffset; i++) {
			int rgb = p[i];
			int a = (rgb >> 24) & 0xff;
			int r = (rgb >> 16) & 0xff;
			int g = (rgb >> 8) & 0xff;
	...
	public static void unpremultiply(int[] p, int offset, int length) {
		...
		for (int i = offset; i < lengthPlusOffset; i++) {
			int rgb = p[i];
			int a = (rgb >> 24) & 0xff;
			int r = (rgb >> 16) & 0xff;
			int g = (rgb >> 8) & 0xff;
